<script>
	import GradientDivider from '../components/GradientDivider.svelte';
</script>

<!-- Investment section starts here -->
<section class="py-16 md:py-24 lg:py-32 relative bg-gradient-to-b from-[#000c14] to-[#000f27]">
	<GradientDivider />
	<div class="container mx-auto">
		<div class="space-y-16 px-4 lg:px-32">
			<div class="flex flex-col items-center gap-4">
				<h2 class="text-white text-5xl font-medium font-cg tracking-wide">
					Global Co-Investment Opportunities
				</h2>
				<p class="text-xl">
					We have access to a large pool of quality deals, thanks to our extensive network and
					ability to collaborate
				</p>
			</div>
			<div
				class="grid sm:grid-cols-3 gap-8 [&>div]:space-y-3 [&>div]:border [&>div]:border-[#00378fc7] [&>div]:rounded-md [&>div]:p-8 [&>div]:bg-gradient-to-tr [&>div]:from-[#00378f6d]"
			>
				<div>
					<h3 class="text-white text-xl font-semibold font-cg tracking-wide">
						Co-Investments & Partnerships
					</h3>
					<p class="text-lg leading-normal text-white/70">
						Capitalizing on our vast global network, we foresee numerous co-investment prospects and
						exclusive sports industry partnerships.
					</p>
				</div>
				<div>
					<h3 class="text-white text-xl font-medium font-cg tracking-wide">Direct Sourcing</h3>
					<p class="text-lg leading-7">
						By actively tracking SportsTech developments and monitoring leading entrepreneurs, we
						strategically approach them for investment at opportune moments.
					</p>
				</div>
				<div>
					<h3 class="text-white text-xl font-medium font-cg tracking-wide">Community Building</h3>
					<p class="text-lg leading-7">
						Focus on building a strong community comprising investment bankers, advisors,
						consultants, angel investors, serial entrepreneurs within the sports tech domain.
					</p>
				</div>
				<div>
					<h3 class="text-white text-xl font-medium font-cg tracking-wide">
						Co-Investments & Partnerships
					</h3>
					<p class="text-lg leading-7">
						Capitalizing on our vast global network, we foresee numerous co-investment prospects and
						exclusive sports industry partnerships.
					</p>
				</div>
				<div>
					<h3 class="text-white text-xl font-medium font-cg tracking-wide">Direct Sourcing</h3>
					<p class="text-lg leading-7">
						By actively tracking SportsTech developments and monitoring leading entrepreneurs, we
						strategically approach them for investment at opportune moments.
					</p>
				</div>
				<div>
					<h3 class="text-white text-xl font-medium font-cg tracking-wide">Community Building</h3>
					<p class="text-lg leading-7">
						Focus on building a strong community comprising investment bankers, advisors,
						consultants, angel investors, serial entrepreneurs within the sports tech domain.
					</p>
				</div>
			</div>
		</div>
	</div>
</section>
<!-- Investment section ends here -->
